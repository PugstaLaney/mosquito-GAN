...
- **DNABERT-2 loading required a custom workaround.** The standard `AutoModelForMaskedLM.from_pretrained()` fails on DNABERT-2 because it uses MosaicBERT architecture and ships a custom `BertConfig` class via `trust_remote_code`. transformers' AutoModel registry enforces a strict type-identity check that treats the custom BertConfig as incompatible with the built-in `BertForMaskedLM`. The fix used in the notebooks bypasses `AutoModel` entirely: `get_class_from_dynamic_module` imports DNABERT-2's own `BertForMaskedLM` class directly from its cached remote code, and the model is instantiated with that class plus the custom config. This produces a clean checkpoint load with all 117M pretrained parameters correctly mapped, rather than the mostly-random initialization that occurs when the standard loading path is used.
